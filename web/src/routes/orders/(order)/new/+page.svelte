<script lang="ts">
	...
	import { generateOrders, order } from '$lib/types/order';
	import { CreateOrderInput } from '../../../../gen/oms/order/v1/orders_pb'
	import Logo from '$lib/components/logo.svelte';

	const onItemClick = async (order: CreateOrderInput) => {
	...
	const onSubmit = async () => {
		if ($order) {
			...
			await fetch('/api/order', {
				method: 'POST',
				body: JSON.stringify({customerId: $order.customerId, id, items: $order.items })
			});
			setTimeout(() => {
