<script lang="ts">
	import type { PaymentSummary } from '../../gen/oms/v1/types_pb';
	import { paymentStatusNames } from '$lib/types/order'
	import StatusBadge from './status-badge.svelte';

	export let payment: PaymentSummary;
</script>
...
<div class="flex flex-col gap-2 mt-4 p-2 rounded bg-gray-200 w-full">
	<div class="flex justify-between">
		...
		<div class="text-right">
			<StatusBadge status={paymentStatusNames.get(payment.status) ?? 'unspecified'} />
		</div>
	</div>
	<div class="flex justify-between">
		<p class="">Subtotal</p>
