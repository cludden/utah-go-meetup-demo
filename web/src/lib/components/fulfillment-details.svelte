<script lang="ts">
	import { fulfillmentStatusNames, shipmentStatusNames } from '$lib/types/order';
	...
	import { Fulfillment, FulfillmentStatus, Order } from '../../gen/oms/v1/types_pb'
	...
	$: getStatus = (fulfillment: Fulfillment) => {
		if (!fulfillment.shipment) return fulfillmentStatusNames.get(fulfillment.status);
		return shipmentStatusNames.get(fulfillment.shipment.status);
	};
</script>
		...
		<div class="container">
			<div class="fulfillment">
				<p class="location">
					...
					{:else if fulfillment?.status == FulfillmentStatus.CANCELLED}
